...
char* indexes(const char* path)
{
    // allocate memory the length of path plus length of file name
    char* p_path = malloc(strlen(path) + 10);
    if (p_path == NULL)
    {
        return NULL;
    }
    char* h_path = malloc(strlen(path) + 11);
    if (h_path == NULL)
    {
        return NULL;
    }
    
    // create and append file names to path strings
    char* php = "index.php";
    ...
    // check to see if path strings exist
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    /* I cannot get the file size correct. Files load, but do not appear as
    ** they should.  I think I should be freading 512 bytes or BYTES at a time,
    ** but I could not get that to work.  I did this at the suggestion of the 
    ** TF.  I originally had a do while and used memcpy
    */
    ...
    // read byte into buffer at pointer to array position [counter]
    while( fread( &buffer[counter], sizeof(BYTE) , 1, file) == 1)
    {
        counter++;
        
        // realloc an additional byte
    ...
    // store buffer and counter into proper return pointers
    *length = counter;
    *content = buffer;
...
const char* lookup(const char* path)
{
    // look for and point to the '.' in path
    char* file_ext = strchr(path, '.');
    
    // compare the file extension to each case
    if(strcasecmp(file_ext, ".css"))
    {
        return "text/css";
    }
    else if(strcasecmp(file_ext, ".html"))
    {
        return "text/html";
    }
    ...
    else if(strcasecmp(file_ext, ".ico"))
    {
        return "image/x-icon";
    }
    ...
    else if(strcasecmp(file_ext, ".js"))
    {
        return "text/javascript";
    }
    else if(strcasecmp(file_ext, ".php"))
    {
        return "text/x-php";
    }
...
{
    // This was produced with in depth help from the TFs at office hours after
    ...
    // copy line and append null terminator
    strcpy(new_line, line);
    new_line[strlen(line)] = '\0';
    
    method = new_line;
    
    // point to first char after first space in new_line
    req_tar = strchr(new_line, space) + 1;
    
    // point to first char after first space in req_tar
    http_version = strchr(req_tar, space) + 1;
    
    int length = strlen(new_line);
    int space_count = 0;
    
    // count space for error checking
    ...
    // look for query (first ?)
    char* que = strchr(req_tar, '?');
    if (que != NULL)
    {
        strcpy(query, que + 1);
        que[0] = '\0';
    ...
    if (que != NULL)
    {
        while (line[i] != ' ')
        {
            i++;
            length_que++;
            printf("length= %i", length_que);
        }
    strncpy(query, que, length_que);
    }
